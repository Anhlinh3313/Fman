var _TypePayCode, _ServiceCode, token, _GetListService;
...
function init_ready() {
    ...
    var allow_edit = ['grid-excel_name_to', 'grid-excel_phone_to', 'grid-excel_address_note_to',
        'grid-excel_address_to', 'grid-excel_cod', 'grid-excel_service', 'grid-excel_type_pay',
        'grid-excel_weight', 'grid-excel_note', 'grid-excel_content'];
...
function init_table(allow_edit, dalidation_columns, dalidation_check) {
    ...
    $(rows).each(function (index, row) {
        ...
        if (index > 0) {
            ...
            oj.fromDistrictId = a.FromDistrictId;
            oj.toDistrictId = a.toDistrictId;
            $.when(sys.CallAjaxPost(apiCustomer + '/price/GetListService',oj, token)).done(function (data) {
                if (data.isSuccess) {
                    _GetListService = data.data;
                }
            });
            validation_row(row);
            //check price
...
function validation_row(data_row) {
    ...
    $(data_row).find('td').each(function (index, cell) {
        ...
        if ($(this).hasClass('not_null')) {
            ...
            var result = validation_number_max_min(value, 0, 100000000);
            success = result[0];
            message = result[1];
        } else if ($(this).hasClass('is_number_min_01')) {
            var result = validation_number_max_min(value, 0.001, 100000000);
            success = result[0];
            message = result[1];
        }
...
function validation_ServiceCode(code) {
    ...
    if (_GetListService != undefined) {
        $.map(_GetListService, function (elementOfArray, indexInArray) {
