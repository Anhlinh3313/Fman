...
function ViewTablePickingList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-picking').val(strDate);
    $('.form-starDate-picking').parent().parent().addClass("active");
    $('.form-EndDate-picking').val(strDate);
    $('.form-EndDate-picking').parent().parent().addClass("active");

    $('.btn-picking').click();
    dtReportPicking = $('#pickinglist').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-picking').val(),
                    toDate: $('.form-EndDate-picking').val(),
                    type: 'Picking',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-picking").on("click", function () {
        dtReportPicking.ajax.reload();
    });
    $(".bt-close-picking").on("click", function () {
        dtReportPicking.clear();
        dtReportPicking.draw();
    })
}