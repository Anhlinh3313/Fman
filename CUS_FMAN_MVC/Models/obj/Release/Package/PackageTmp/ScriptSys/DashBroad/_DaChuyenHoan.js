var dtReportDaChuyenHoan;
$(function () {

    //GetStatusLading();
});

function ViewTableDaChuyenHoanList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-dachuyenhoan').val(strDate);
    $('.form-starDate-dachuyenhoan').parent().parent().addClass("active");
    $('.form-EndDate-dachuyenhoan').val(strDate);
    $('.form-EndDate-dachuyenhoan').parent().parent().addClass("active");

    $('.btn-dachuyenhoan').click();
    dtReportDaChuyenHoan = $('#DaChuyenHoan').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-dachuyenhoan').val(),
                    toDate: $('.form-EndDate-dachuyenhoan').val(),
                    type: 'TransferList',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-dachuyenhoan").on("click", function () {
        dtReportDaChuyenHoan.ajax.reload();
    });
    $(".bt-close-dachuyenhoan").on("click", function () {
        dtReportDaChuyenHoan.clear();
        dtReportDaChuyenHoan.draw();
    })
}