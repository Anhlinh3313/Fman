var dtReportChoChuyenHoan;
$(function () {

    //GetStatusLading();
});

function ViewTableChoChuyenHoanList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-chochuyenhoan').val(strDate);
    $('.form-starDate-chochuyenhoan').parent().parent().addClass("active");
    $('.form-EndDate-chochuyenhoan').val(strDate);
    $('.form-EndDate-chochuyenhoan').parent().parent().addClass("active");

    $('.btn-chochuyenhoan').click();
    dtReportChoChuyenHoan = $('#ChoChuyenHoan').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-chochuyenhoan').val(),
                    toDate: $('.form-EndDate-chochuyenhoan').val(),
                    type: 'ReturnList',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-chochuyenhoan").on("click", function () {
        dtReportChoChuyenHoan.ajax.reload();
    });
    $(".bt-close-chochuyenhoan").on("click", function () {
        dtReportChoChuyenHoan.clear();
        dtReportChoChuyenHoan.draw();
    })
}