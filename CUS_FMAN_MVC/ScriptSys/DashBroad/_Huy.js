...
function ViewTableHuyList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-huy').val(strDate);
    $('.form-starDate-huy').parent().parent().addClass("active");
    $('.form-EndDate-huy').val(strDate);
    $('.form-EndDate-huy').parent().parent().addClass("active");

    $('.btn-huy').click();
    dtReportHuy = $('#Huy').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function(d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-huy').val(),
                    toDate: $('.form-EndDate-huy').val(),
                    type: 'Cancel',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-huy").on("click", function() {
        dtReportHuy.ajax.reload();
    });
    $(".bt-close-huy").on("click", function() {
        dtReportHuy.clear();
        dtReportHuy.draw();
    })
}