var dtReportDelivering;
$(function () {

    //GetStatusLading();
});

function ViewTableDeliveringList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-delivering').val(strDate);
    $('.form-starDate-delivering').parent().parent().addClass("active");
    $('.form-EndDate-delivering').val(strDate);
    $('.form-EndDate-delivering').parent().parent().addClass("active");

    $('.btn-delivering').click();
    dtReportDelivering = $('#Delivering').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-delivering').val(),
                    toDate: $('.form-EndDate-delivering').val(),
                    type: 'Delivering',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-delivering").on("click", function () {
        dtReportDelivering.ajax.reload();
    });
    $(".bt-close-delivering").on("click", function () {
        dtReportDelivering.clear();
        dtReportDelivering.draw();
    })
}