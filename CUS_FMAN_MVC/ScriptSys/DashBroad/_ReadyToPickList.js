var dtReportReadyToPickList;
$(function () {

    //GetStatusLading();
});

function ViewTableReadyToPickList() {
    var date = new Date();
    var strDate = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
    $('.form-starDate-readytopick').val(strDate);
    $('.form-starDate-readytopick').parent().parent().addClass("active");
    $('.form-EndDate-readytopick').val(strDate);
    $('.form-EndDate-readytopick').parent().parent().addClass("active");

    $('.btn-readytopick').click();
    dtReportReadyToPickList = $('#ReadyToPickList').DataTable({
        ordering: true,
        bFilter: true,
        ...
        ajax: {
            ...
            data: function (d) {
                return {
                    pageSize: d.length,
                    pageNumber: d.draw,
                    fromDate: $('.form-starDate-readytopick').val(),
                    toDate: $('.form-EndDate-readytopick').val(),
                    type: 'ReadyToPick',
                    cols: 'ShipmentStatus'
                }
    ...
    $(".btn-search-readytopick").on("click", function () {
        dtReportReadyToPickList.ajax.reload();
    });
    $(".bt-close-readytopick").on("click", function () {
        dtReportReadyToPickList.clear();
        dtReportReadyToPickList.draw();
    })
}